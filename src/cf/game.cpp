...
#define G_BUTTON_LANGUAGE_OPTIONS 16
#define G_BUTTON_VIDEO_OPTIONS	 17
#define G_BUTTON_SOUND_OPTIONS	 18
#define G_BUTTON_ABORT		 19
#define G_BUTTON_QUIT		 20

#define G_BUTTON_SHUTDOWN	 21

#define G_BUTTON_UNIT_INFO	 22
#define G_BUTTON_UNIT_CONTENT	 23
#define G_BUTTON_UNIT_SWEEP	 24
#define G_BUTTON_UNIT_UNDO	 25

#define G_KEY_QUIT		0
#define G_KEY_INFO		1
#define G_KEY_MAP		2
#define G_KEY_END_TURN		3
#define G_KEY_CONTENT		4
#define G_KEY_SWEEP		5
...
void Game::InitKeys( void ) {
  ...
  for ( int i = 0; i <= G_KEY_SWEEP; ++i ) {
    ...
    if ( pos != string::npos ) keys[i] = tolower(label[pos+1]);
    else keys[i] = 0;
  }
}
...
Unit *Game::MoveUnit( Unit *u, const Point &dest ) {
  if ( u->Position() != dest ) {
    ...
    if ( mission->GetPlayer().IsHuman() ) {
      ...
      if ( u->IsTransport() && u->IsSheltered() ) {
        ...
        UnitLoadWindow *ulw = new UnitLoadWindow(
            ...
            *map->GetUnitSet(), *map->GetTerrainSet(), view );
    ...
    if ( conquer == 1 ) {              // a building was conquered
      ...
      if ( h ) h->RecordTileEvent( map->HexTypeID( posnew ), oldhex, posnew.x, posnew.y );
    }
...
void Game::Undo( void ) {
  ...
  if ( mission->GetHistory() )
    mission->GetHistory()->EraseMoveEvents( *u );
  ...
  if ( event.type == SDL_KEYDOWN ) {
    ...
    case SDLK_KP8: case SDLK_KP9:
      if ( !(event.key.keysym.mod & KMOD_NUM) ) {
        ScrollCommand( event.key.keysym.sym );
        break;
      }
      // fall through
    ...
    case SDLK_F1:        // game menu
      GameMenu();
      break;
    default:
      // these shortcuts change with the locale
      short key = event.key.keysym.sym;

      if ( key == keys[G_KEY_INFO] ) {
        Unit *u = map.GetUnit( mv->Cursor() );
        if ( u ) UnitInfo( u );
      } else if ( key == keys[G_KEY_MAP] ) {
        new TacticalWindow( mv, *mission, view );
      } else if ( key == keys[G_KEY_CONTENT] ) {
      ...
      } else if ( key == keys[G_KEY_END_TURN] ) {
        rc = EndTurn();
      } else if ( key == keys[G_KEY_SWEEP] ) {
        if ( unit && MinesweeperTargets( unit ) )
          EnterSpecialMode( MODE_SWEEP );
      } else if ( key == keys[G_KEY_QUIT] ) {
        Quit();
      }
      break;
    }
