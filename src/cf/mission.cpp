...
Unit *Mission::LoadUnit( File &file, bool dummy /* = false */ ) {
...
int Mission::QuickLoad( File &file ) {
  int rc = -1;
...
Unit *Mission::CreateUnit( unsigned char type, Player &p,
  ...
  if ( history ) history->RecordUnitEvent( *u );
...
void Mission::SetLocale( const string &lang ) {
  ...
  for ( Building *b = static_cast<Building *>(shops.Head());
    ...
    b->SetName( messages.GetMsg(b->NameID()) );
}
