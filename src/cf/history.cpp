...
int HistEvent::Save( File &file ) const {
  file.Write8( type );
...
short History::Load( File &file, Mission &mission ) {
...
int History::Save( File &file ) const {
...
// DESCRIPTION: Record the creation of a new unit.
// PARAMETERS : unit - newly created unit
// RETURNS    : 0 on success, -1 on error
////////////////////////////////////////////////////////////////////////

int History::RecordUnitEvent( const Unit &unit ) {
  HistEvent *he = new HistEvent;

  he->type = HIST_UNIT;
  he->data[0] = unit.ID();
  he->data[1] = he->data[2] = he->data[3] = -1;
...
void History::ReplayAttackEvent( const HistEvent &event, MapWindow *mapwin ) {
  ...
  if ( u ) {
    ...
    upd = mv->SetCursor( Point(event.data[1], event.data[2]) );
    mapwin->Show( upd );
    mv->SetCursor( Point(-1,-1) );
    SDL_Delay( delay );
    upd = mv->UpdateHex( u->Position() );
    mapwin->Show( upd );
    upd = mv->UpdateHex( Point(event.data[1], event.data[2]) );
    mapwin->Show( upd );
    lastunit = u;
  }
}
...
void History::ReplayCombatEvent( const HistEvent &event, MapWindow *mapwin ) {
  ...
  cmb.aamod = (signed char)event.data[3];
  cmb.ddmod = (signed char)event.data[2];

  Gam->ResolveBattle( &cmb );
}
...
////////////////////////////////////////////////////////////////////////
// NAME       : History::EraseMoveEvents
// DESCRIPTION: Erase all recorded movement for a spcific unit.
// PARAMETERS : u - unit to erase movement for
// RETURNS    : -
////////////////////////////////////////////////////////////////////////

void History::EraseMoveEvents( const Unit &u ) {
  HistEvent *he = static_cast<HistEvent *>( events.Head() ), *tmp;

  while ( he ) {
    tmp = static_cast<HistEvent *>( he->Next() );

    if ( (he->type == HIST_MOVE) && (he->data[0] == u.ID()) ) {
      he->Remove();
      delete he;
    }
