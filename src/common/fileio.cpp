...
unsigned char File::Read8( void ) const {
  unsigned char val;
  Read( &val, 1 );
  return val;
}
...
string File::ReadS( int size ) const {
  string str;
...
////////////////////////////////////////////////////////////////////////
// NAME       : File::WriteS
// DESCRIPTION: Read a string to the file.
...
int File::WriteS( string value, int len /* = 0 */ ) const {
  int size = value.size();
  int rc = SDL_RWwrite(fh, value.c_str(), value.size(), 1);

  if ( size < len ) {
    for ( ; len > size; --len ) Write8( 0 );
  }
  return rc;
}
...
# else
  char dir[MAX_PATH];
  int length = GetEnvironmentVariable((LPCWSTR)"HOME", (LPWSTR)dir, MAX_PATH);
  if ( length == 0 ) {
    length = GetEnvironmentVariable((LPCWSTR)"HOMEDRIVE", (LPWSTR)dir, MAX_PATH);
    length += GetEnvironmentVariable((LPCWSTR)"HOMEPATH", (LPWSTR)(dir + length), MAX_PATH - length);
  }
